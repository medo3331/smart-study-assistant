...
describe("Document Analyzer Agent", () => {
  ...
  it("Arabic document content", async () => {
    const r = await documentAnalyzerAgent({ prompt: "لخص هذا", context: { language: "ar", preferences: { content: "الدروس: الفيزياء الأساسية.", subject: "physics" } } }, () =>
      Promise.resolve({ ok: true, agent: "document_analyzer", provider: "nvidia", model: "test", content: "ملخص الدرس: قوانين نيوتن...", retryable: false } as unknown as AgentResult)
    );
  ...
  it("English document content", async () => {
    const r = await documentAnalyzerAgent({ prompt: "Explain this chapter", context: { language: "en", preferences: { content: "Chapter 3: Calculus", subject: "math" } } }, () =>
      Promise.resolve({ ok: true, agent: "document_analyzer", provider: "nvidia", model: "test", content: "Chapter summary and key points.", retryable: false } as unknown as AgentResult)
    );
    expect(r.ok).toBe(true);
  });
  it("summarization", async () => {
    const r = await documentAnalyzerAgent({ prompt: "Summarize", context: { preferences: { content: "Long text..." } } }, () =>
      Promise.resolve({ ok: true, agent: "document_analyzer", provider: "nvidia", model: "test", content: "Summary provided.", retryable: false } as unknown as AgentResult)
    );
    expect(r.ok).toBe(true);
  });
  it("extraction / key points", async () => {
    const r = await documentAnalyzerAgent({ prompt: "Extract main points", context: { preferences: { content: "A B C D E" } } }, () =>
      Promise.resolve({ ok: true, agent: "document_analyzer", provider: "nvidia", model: "test", content: "Key points: A, B, C.", retryable: false } as unknown as AgentResult)
    );
    expect(r.ok).toBe(true);
  });
  it("missing/invalid document asks", async () => {
    const r = await documentAnalyzerAgent({ prompt: "Analyze", context: { preferences: {} } }, () =>
      Promise.resolve({ ok: false, agent: "document_analyzer", provider: "nvidia", code: "MISSING_INPUT", message: "Provide document content.", retryable: true } as unknown as AgentResult)
    );
  ...
  it("vision/input shape", async () => {
    const r = await documentAnalyzerAgent({ prompt: "Analyze image", context: { preferences: { imageInput: "base64", subject: "art" } } }, () =>
      Promise.resolve({ ok: true, agent: "document_analyzer", provider: "nvidia", model: "test", content: "Image shows diagram.", retryable: false } as unknown as AgentResult)
    );
    expect(r.ok).toBe(true);
  });
  it("provider error MODEL_404", async () => {
    const r = await documentAnalyzerAgent({ prompt: "Q", context: { preferences: { content: "x" } } }, () =>
      Promise.resolve({ ok: false, agent: "document_analyzer", provider: "nvidia", code: "MODEL_404", message: "404", retryable: true } as unknown as AgentResult)
    );
    expect(r.ok).toBe(false);
