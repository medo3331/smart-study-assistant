...
describe("Freelance Assistant Agent", () => {
  ...
  it("student to freelance transition", async () => {
    const r = await freelanceAgent({ prompt: "How to start as freelancer", context: { role: "student", preferences: { field: "cs", level: "grad" } } }, () =>
      Promise.resolve({ ok: true, agent: "career", provider: "nvidia", model: "test", content: "Build portfolio -> set pricing -> find niche.", retryable: false } as any)
    );
  ...
  it("graduate freelance path", async () => {
    const r = await freelanceAgent({ prompt: "Career as freelancer after graduation", context: { role: "graduate", preferences: { field: "design" } } }, () =>
      Promise.resolve({ ok: true, agent: "freelance", provider: "nvidia", model: "test", content: "Build niche portfolio, set rates, manage clients.", retryable: false } as any)
    );
    expect(r.ok).toBe(true);
  });
  ...
  it("multilingual ar", async () => {
    const r = await freelanceAgent({ prompt: "كيف أبدأ كمستقل؟", context: { language: "ar" } }, () =>
      Promise.resolve({ ok: true, agent: "freelance", provider: "nvidia", model: "test", content: "ابدأ بتحديد الاختصاص ثم بناء الأعمال.", retryable: false } as any)
    );
    expect(r.ok).toBe(true);
  });
  it("multilingual en", async () => {
    const r = await freelanceAgent({ prompt: "Plan my freelance move", context: { language: "en" } }, () =>
      Promise.resolve({ ok: true, agent: "freelance", provider: "nvidia", model: "test", content: "Plan: niche → portfolio → pricing → clients.", retryable: false } as any)
    );
    expect(r.ok).toBe(true);
  });
  it("missing context asks", async () => {
    const r = await freelanceAgent({ prompt: "Help", context: { preferences: {} } }, () =>
      Promise.resolve({ ok: false, agent: "freelance", provider: "nvidia", code: "MISSING_INPUT", message: "Provide role, field, goals.", retryable: true } as any)
    );
    expect(r.ok).toBe(false);
  });
  ...
  it("no invented job/currency/source", async () => {
    const r = await freelanceAgent({ prompt: "Find job listings", context: { preferences: {} } }, () =>
      Promise.resolve({ ok: true, agent: "freelance", provider: "nvidia", model: "test", content: "I don't invent job openings — provide your profile and target for guidance.", retryable: false } as any)
    );
    expect(r.ok).toBe(true);
    expect(r.content || "").not.toContain("Google");
  });
});
