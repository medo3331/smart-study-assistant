...
/* ==========================================================================
   ...
   خلفية ‎#07091A · كروت زجاجية ‎#0D1029@70% · حدود white/[0.06]
...
export function LessonShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative min-h-screen" dir="rtl">
      ...
      <div aria-hidden className="fixed inset-0 -z-10 bg-[#07091A]" />
      <div
        aria-hidden
        className="pointer-events-none fixed inset-0 -z-10"
        style={{
          background:
            "radial-gradient(52% 42% at 85% -4%, rgba(124,92,255,0.13) 0%, transparent 62%), radial-gradient(40% 34% at 6% 100%, rgba(45,212,191,0.07) 0%, transparent 60%)",
        }}
      />
...
.magicly-lesson .sheet-card-live{border-top:1px solid rgba(124,92,255,.25)}
.magicly-lesson .eyebrow{color:#B69CFF}
...
.magicly-lesson .btn-marker{background:#7C5CFF;border-color:#7C5CFF;color:#fff;box-shadow:0 6px 20px rgba(124,92,255,.35)}
.magicly-lesson .btn-marker:hover{background:#8E72FF}
.magicly-lesson .meter{background:rgba(255,255,255,.07)}
.magicly-lesson .meter-fill{background:linear-gradient(to left,#B69CFF,#7C5CFF)}
...
export function GlassCard({ children, className, glow }: GlassCardProps) {
  return (
    <div
      className={
        "relative overflow-hidden rounded-[24px] border border-white/[0.06] bg-[#0D1029]/70 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.35)]" +
        (className ? ` ${className}` : "")
      }
    >
      {glow && (
        <span
          ...
          style={{
            background:
              "linear-gradient(to left, transparent, rgba(124,92,255,0.55), transparent)",
          }}
        />
      )}
      {children}
