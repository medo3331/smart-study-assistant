"use client";
/* eslint-disable react-hooks/set-state-in-effect -- Secondary subject sync is intentional */
...
const EXAMPLES = [
  ...
  "عايز أحل امتحان",
  "عايز أتدرب",
  "عايز أعمل خطة مذاكرة",
];

export function SecondaryGoalInput({ onRecommendation, gradeName, trackName, externalGoal, onExternalGoalConsumed, onOpenAi, onOpenLesson }: Props) {
  ...
  async function submit(g: string) {
    ...
    try {
      const res = await fetch("/api/secondary/goal", {
        method: "POST",
      ...
      setError("حصل خلل بسيط. جرّب تاني أو اختر مادة");
      setResult({
        intent: "general",
        message: "حدد مادتك ونبدأ بخطة مركزة",
        recommendedActions: [
          { capability: "study", label: "ذاكر الدرس", href: "/dashboard", reason: "شرح أكاديمي" },
          { capability: "exam", label: "حل امتحان", href: "/exams", reason: "محاكاة" },
        ],
      });
      console.error("[SecondaryGoalInput]", msg);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleAction(cap: string, label: string) {
    ...
    if (cap === "study" || cap === "practice" || cap === "quiz" || cap === "exam") {
      const ctx = `${gradeName || "الثانوي"}${trackName ? ` ${trackName}` : ""}`;
      const promptMap: Record<string, string> = {
        study: `اشرحلي ${baseGoal || "الدرس"} للصف ${ctx} بأسلوب أكاديمي مركز مع مثال تطبيقي`,
        practice: `راجعلي ${baseGoal || "الدرس"} للصف ${ctx} مع خلاصة ونقاط مهمة`,
        quiz: `اعملي 4 أسئلة متدرجة عن ${baseGoal || "الدرس"} للصف ${ctx} مع إجابات نموذجية`,
        exam: `اعملي امتحان 5 أسئلة شامل عن ${baseGoal || "الدرس"} للصف ${ctx} مع نموذج إجابة`,
      };
      ...
      try {
        const res = await fetch("/api/ai", {
          ...
          body: JSON.stringify({
            ...
            user: { role: "student", language: "ar", educationLevel: `secondary ${ctx}` },
          }),
        });
        const json = await res.json();
    ...
    if (cap === "study_plan") {
      setExplainTitle("خطة مقترحة");
      setExplainContent("• خصص 50 دقيقة لكل مادة\n• حل أسئلة بعد كل درس\n• مراجعة يومية 20 دقيقة\n• امتحان أسبوعي شامل");
    }
  }
    ...
    <section aria-label="هدف اليوم" className="sheet-card overflow-hidden">
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-display text-[1.03rem] font-semibold text-ink">ماذا تريد أن تنجز اليوم؟</h2>
            <p className="mt-1 text-sm leading-6 text-ink-soft">هدف واضح يقود جلسة مذاكرة فعالة.</p>
          </div>
        ...
        <div className="mt-4 flex gap-2">
          <label htmlFor="secondary-goal-input" className="sr-only">هدف اليوم</label>
          <input
            id="secondary-goal-input"
            value={goal}
            onChange={(e) => setGoal(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter" && !loading) void submit(goal); }}
            placeholder="مثال: عايز أذاكر الفيزياء"
