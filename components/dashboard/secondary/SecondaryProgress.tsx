...
export function SecondaryProgress({ completed, total, progressPct, currentDay }: Props) {
  if (!total || total === 0) {
    return (
      <section aria-label="التقدم" className="sheet-card p-5 sm:p-6">
        ...
        <div className="mt-4 rounded-xl border border-dashed border-rule bg-paper-2 p-6 text-center">
          <p className="text-sm font-semibold text-ink">ابدأ أول درس لتتبع تقدمك</p>
          <p className="mt-1 text-xs text-ink-soft">سيظهر هنا إنجازك ونسبة الإتمام</p>
        </div>
      </section>
    );
  }
